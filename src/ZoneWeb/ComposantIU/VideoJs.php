...
class VideoJs extends PvComposantJSFiltrable
{
	...
	{
		parent::InitConfig() ;
		$this->AttrsTag = new \Pv\ZoneWeb\ComposantIU\AttrsTagVideoJs() ;
	}
	...
	protected function RenduDispositifBrutSpec()
	{
		...
		if(count($this->ElementsEnCours) > 0)
		{
			...
			$ctn .= '>'.PHP_EOL ;
			$extsTypesMime = array_keys(\Pv\ZoneWeb\ComposantIU\VideoJs::$TypesMimeDefaut) ;
			foreach($this->ElementsEnCours as $i => $lgn)
			{
				...
				$ctn .= '<source src="'.htmlspecialchars($cheminVideo).'" title="'.htmlspecialchars($titre).'" type="'.\Pv\ZoneWeb\ComposantIU\VideoJs::$TypesMimeDefaut[$extension].'" >'.PHP_EOL ;
			}
